# Plotting good seq tech PARs
## Filter /data/Wilson_Lab/projects/VGP_Phase_1_Sex_Chr_Project/jacksondan/datafiles/VGP_OrdinalList_Phase1Freeze_v1.1_sex_chroms_seqCov_HalfDeep_SCINKD_May8.25_DwnldDec16.25.csv to just those with good assembly tech

```
Rscript filter_on_assemblytech.R
```
### Output:
```
GOOD         242  -> /data/Wilson_Lab/projects/VGP_Phase_1_Sex_Chr_Project/jacksondan/referencelists/VGP_OrdinalList_Phase1Freeze_v1.1_goodAssemblytech.csv
POOR         337  -> /data/Wilson_Lab/projects/VGP_Phase_1_Sex_Chr_Project/jacksondan/referencelists/VGP_OrdinalList_Phase1Freeze_v1.1_poorAssemblytech.csv
UNCERTAIN     88  -> /data/Wilson_Lab/projects/VGP_Phase_1_Sex_Chr_Project/jacksondan/referencelists/VGP_OrdinalList_Phase1Freeze_v1.1_uncertainAssemblytech.csv
```

## Plot good assembly tech
```
...
filter_file <- "/data/Wilson_Lab/projects/VGP_Phase_1_Sex_Chr_Project/jacksondan/referencelists/VGP_OrdinalList_Phase1Freeze_v1.1_goodAssemblytech.csv"
filter_df <- read_csv(filter_file, col_types = cols(.default = col_character()), na = c("", "NA", "NaN"))
...
# --- filter files to species present in filter_df$Scientific.Name ---

# species list from filter_df (trim + unique)
keep_species <- filter_df$Scientific.Name %>%
  str_squish() %>%
  unique()

# extract species from filename, e.g. "Taeniopygia_guttata" -> "Taeniopygia guttata"
# species is everything between last "/" and last "_" in the filename
file_species <- basename(files) %>%
  str_replace("\\.[^.]*$", "") %>%   # drop final extension (.csv) just in case
  str_remove("_[^_]*$") %>%          # drop from LAST underscore to end (e.g., _WtoZ.aln.refqry)
  str_replace_all("_", " ") %>%
  str_squish()
...
files_filt <- files[file_species %in% keep_species]

cat("Total files:", length(files), "\n")
cat("Files after filter:", length(files_filt), "\n")
...
out_tbl_qry <- sprintf("surviving_regions.qry.thr%s.len%d.goodAssemblytech.csv", thr_tag, min_aln)
out_tbl_ref <- sprintf("surviving_regions.ref.thr%s.len%d.goodAssemblytech.csv", thr_tag, min_aln)
...
out_png <- sprintf(
  "continuous_percentID.ALLFILES.with_tree_and_chr_boxes.thr%s.len%d.goodAssemblytech.png",
  thr_tag,
  min_aln
)
ggsave(
...
filter_file <- "/data/Wilson_Lab/projects/VGP_Phase_1_Sex_Chr_Project/jacksondan/referencelists/VGP_OrdinalList_Phase1Freeze_v1.1_poorAssemblytech.csv"
filter_df <- read_csv(filter_file, col_types = cols(.default = col_character()), na = c("", "NA", "NaN"))
...
out_tbl_qry <- sprintf("surviving_regions.qry.thr%s.len%d.poorAssemblytech.csv", thr_tag, min_aln)
out_tbl_ref <- sprintf("surviving_regions.ref.thr%s.len%d.poorAssemblytech.csv", thr_tag, min_aln)
...
out_png <- sprintf(
  "continuous_percentID.ALLFILES.with_tree_and_chr_boxes.thr%s.len%d.poorAssemblytech.png",
  thr_tag,
  min_aln
)
ggsave(
