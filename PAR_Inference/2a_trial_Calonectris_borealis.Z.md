...
Spot check a couple genomes on the ASU Sol Cluster

scp /Users/danjack/Downloads/VGP_freeze_hap1_combined_sexchroms_seq_reports.tsv dnjacks4@login.sol.rc.asu.edu:/scratch/dnjacks4/TE_PAR_Inference/reference_lists

ssh dnjacks4@login.sol.rc.asu.edu

cd /scratch/dnjacks4/
mkdir TE_PAR_Inference
cd TE_PAR_Inference
mkdir datafiles reference_lists analyses analyses/spot_test

cd /scratch/dnjacks4/TE_PAR_Inference/analyses/spot_test
...
grep 'OZ122106.1' ${BEDDIR}/Calonectris_borealis_GCA_964195595.2.repMask.out.bed > /scratch/dnjacks4/TE_PAR_Inference/datafiles/Calonectris_borealis.GCA_964195595.2.TE.Z.bed

grep -v 'CAXM' ${BEDDIR}/Calonectris_borealis_GCA_964195595.2.repMask.out.bed | grep -v 'OZ122106.1' | grep -v 'OZ077850.1' | grep -v '	OZ122107.1' > /scratch/dnjacks4/TE_PAR_Inference/datafiles/Calonectris_borealis.GCA_964195595.2.TE.autosomes.bed


TE_BED=/scratch/dnjacks4/TE_PAR_Inference/datafiles/Calonectris_borealis.GCA_964195595.2.TE.autosomes.bed
Z_TE_BED=/scratch/dnjacks4/TE_PAR_Inference/datafiles/Calonectris_borealis.GCA_964195595.2.TE.Z.bed

# Autosomes
awk 'BEGIN{OFS="\t"} {print $1, $2, $3, $3-$2}' "$TE_BED" > autosomes.te.bed

# Z chromosome
awk 'BEGIN{OFS="\t"} {print $1, $2, $3, $3-$2}' "$Z_TE_BED" > Z.te.bed


# --------------------------
# Z windows (your approach)
# --------------------------
echo -e 'OZ122106.1\t87715099' > zchrom.size
bedtools makewindows -g zchrom.size -w 50000 > Z.50kb.bed

bedtools intersect -a Z.50kb.bed -b Z.te.bed -wa -wb |
  awk 'BEGIN{OFS="\t"} {print $1,$2,$3,$7}' |
  bedtools groupby -g 1,2,3 -c 4 -o sum > Z.TEbp.50kb.bed

# Add zero-TE Z windows
bedtools intersect -a Z.50kb.bed -b Z.TEbp.50kb.bed -v |
  awk 'BEGIN{OFS="\t"} {print $1,$2,$3,0}' >> Z.TEbp.50kb.bed
...
p1 <- ggplot(Z, aes(x = mb, y = prop)) +
  ...
  labs(
    ...
    shape = "FDR < 0.05",
    title = "TE density along Z (50 kb windows)",
    subtitle = sprintf(
...
ggsave(
  filename = "Z_TE_density_50kb.png",
  plot = p1,
  width = 10,
  height = 4,
  units = "in",
...
p2 <- ggplot(Z, aes(x = mb, y = mlog10_fdr)) +
  ...
  labs(
    ...
    shape = "FDR < 0.05",
    title = "Windows with significantly low TE density on Z",
    subtitle = "Dashed line = FDR 0.05 threshold"
  ) +
  theme_classic()

ggsave(
  filename = "Z_TE_depletion_FDR_50kb.png",
  plot = p2,
  width = 10,
  height = 4,
  units = "in",
...
scp "dnjacks4@login.sol.rc.asu.edu:/scratch/dnjacks4/TE_PAR_Inference/analyses/spot_test/*.png" .
