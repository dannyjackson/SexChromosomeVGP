...
function process_gtf(gtf, species, chr, par_start, par_stop,    cmd, gline, f, n, start, stop, gene_id, transcript_id, gene, toga_dirname, gtf_chr, key, k, status) {
    ...
    while ((cmd | getline gline) > 0) {
        ...
        # Keep records where column 1 contains chrX.
        # This accepts chrX, chrX_pat_hsa1, chrX_hap1_hsa1, etc.
        if (gtf_chr !~ /chrX/) {
            continue
        }
        ...
        # Use gene rows for gene coordinates
        if (f[3] == "gene") {
            g_start[key] = start
            g_stop[key] = stop
            g_chr[key] = f[1]
        ...
        # Prefer gene_name if present
        gene = attr_value(f[9], "gene_name")

        # Otherwise extract symbol from TOGA transcript_id:
        # ENST00000326153.10#GTPBP6#3 -> GTPBP6
        if (gene == "" && transcript_id ~ /#/) {
            split(transcript_id, tparts, "#")
            gene = tparts[2]
        }

        # Last fallback: cleaned gene_id
        if (gene == "") {
            gene = gene_id
            sub(/^lost_/, "", gene)
            sub(/^retro_/, "", gene)
        ...
        # Do not overwrite a real symbol like GTPBP6 with ENSG...
...

{
    ...
    #
    # The chromosome accession in the input is still used to get the PAR region,
    # but GTF records are now selected by whether column 1 contains chrX.
