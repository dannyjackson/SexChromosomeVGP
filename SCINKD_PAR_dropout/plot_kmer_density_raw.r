...
hap1_kmer <- read.delim("Homo_sapiens.hap1-minus-hap2.results", header = F)
hap2_kmer <- read.delim("Homo_sapiens.hap2-minus-hap1.results", header = F)

hap1_idx <- read.delim("Homo_sapiens.hap1.fasta.gz.fai", header = F)
hap2_idx <- read.delim("Homo_sapiens.hap2.fasta.gz.fai", header = F)

hap1_bed <- read.delim("Homo_sapiens.hap1-minus-hap2.bed", header = F)
hap2_bed <- read.delim("Homo_sapiens.hap2-minus-hap1.bed", header = F)
...
#mirror plot
mirror <- ggplot(hap1_bed, aes(x=start)) +
  geom_histogram(data = hap1_bed,  aes(x = start, y =  after_stat(density)), color = "#5F00F6", fill = "black", alpha = 0.8, binwidth=500000) +
  geom_histogram(data = hap2_bed, aes(x = start, y = -after_stat(density)), color = "#62CD32", fill = "grey", alpha = 0.8, binwidth=500000) +
  ggtitle("Kmer Density: X (above) vs Y (below)") +
  xlab("Position on chromosome") + 
  ylab("Kmer density") + 
  theme_bw()


out_file <- "kmer_raw.500k.png"
...
Xchr <- ggplot(hap1_bed, aes(x=start)) +
  geom_histogram(data = hap1_bed,  aes(x = start, y =  after_stat(density)), color = "#5F00F6", fill = "black", alpha = 0.8, binwidth=500000) +
  ggtitle("Kmer Density of X chr (raw Y)") +
  xlab("Position on chromosome") + 
  ylab("Kmer density") + 
  theme_bw()

out_file <- "kmer_raw.Xchr.500k.png"
