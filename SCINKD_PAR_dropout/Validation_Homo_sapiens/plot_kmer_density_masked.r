...
hap1_kmer <- read.delim("Homo_sapiens.masked.hap1-minus-hap2.results", header = F)
hap2_kmer <- read.delim("Homo_sapiens.masked.hap2-minus-hap1.results", header = F)

hap1_idx <- read.delim("Homo_sapiens.masked.hap1.fasta.gz.fai", header = F)
hap2_idx <- read.delim("Homo_sapiens.masked.hap2.fasta.gz.fai", header = F)

hap1_bed <- read.delim("Homo_sapiens.masked.hap1-minus-hap2.bed", header = F)
hap2_bed <- read.delim("Homo_sapiens.masked.hap2-minus-hap1.bed", header = F)
...
mirror <- ggplot(hap1_bed, aes(x=start)) +
  ...
  ggtitle("Kmer Density: X (above) vs Y_PAR_masked (below)") +
  xlab("Position on chromosome") + 
  ylab("Kmer density") + 
  theme_bw()


out_file <- "kmer_masked.png"
