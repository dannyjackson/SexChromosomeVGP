...
par_orientation <- df %>%
  ...
  summarise(
    min_PAR_start = min(Start_pos, na.rm = TRUE),
    PAR_at_end = min_PAR_start >= 10000000,
    .groups = "drop"
  )
...
par_genes <- df %>%
  ...
  left_join(par_orientation, by = c("Species", "Chrom")) %>%
  mutate(
    PAR_at_end = replace_na(PAR_at_end, FALSE),
