...
GENOME_DIR="/data/Wilson_Lab/data/VGP_genomes_phase1/Alternate_Haplotypes"
...
header="$(head -n 1 "$ACCESSION_LIST" | sed '1s/^\xEF\xBB\xBF//' | tr -d '\r')"
...
sci_col=$(
  awk -v FS=',' -v h="$header" 'BEGIN{
    ...
    for(i=1;i<=n;i++){
      ...
      if(a[i]=="Scientific.Name"){print i; exit}
    }
  }'
)
...
acc_col=$(
  awk -v FS=',' -v h="$header" 'BEGIN{
    ...
    for(i=1;i<=n;i++){
      gsub(/^[[:space:]]+|[[:space:]]+$/,"",a[i]);
      if(a[i]=="Alternate_Haplotype"){print i; exit}
    }
  }'
)
...
acc_col=$(
  awk -v FS=',' -v h="$header" 'BEGIN{
    n=split(h,a,FS);
    for(i=1;i<=n;i++) if(a[i]=="Alternate_Haplotype"){print i; exit}
  }'
)
