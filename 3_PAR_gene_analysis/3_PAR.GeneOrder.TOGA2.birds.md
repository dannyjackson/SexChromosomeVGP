...
```
# Find gaps within the PAR introduced by Hi-C joining
```
../mammals/2a_find_PAR_gaps.sh

Rscript ../mammals/2b_plot_PAR_gaps.R
...
This list matches the formatting of each chromosome in the TOGA2 annotation dataset
```
...
```
## PAR boundaries with 98.5 PID and 10kb limits 
This list includes the version suffix of each chromosome
```
...
EOF
```
# Identify location of each PAR gene using the TOGA annotations
```
export TOGA_DIR="/data/Wilson_Lab/data/TOGA2_Hiller/zebrafinch_HLtaeGut5-GCF_003957565.2" 

chmod +x 3a_find_par_genes_toga.birds.awk 
./3a_find_par_genes_toga.birds.awk PAR.species_chr_region.txt > birds_PAR_genes.tsv
...
# Remove LOC genes and fix errors in naming
...
# Create the plot
```
Rscript 3b_PAR_GeneOrder_Birds.August2026.R
```