...
OUT_PREFIX <- paste0(
    tolower(SPECIES),
    "_PAR_genes_repeats_percentID.eachclass"
)
...
rep[, repeat_plot_class := fifelse(
    repeat_class == "DNA/hAT-Charlie",
    "hAT-Charlie",
    as.character(broad_class)
)]
...
repeat_colors <- c(
    ...
    "LTR"             = "#984ea3",
    "DNA"             = "#ff7f00",
    "hAT-Charlie"     = "#00A6FF",  # bright blue
    "RC/Helitron"     = "#a65628",
    "Retroposon"      = "#f781bf",
    "PLE"             = "#999999",
    "Satellite"       = "#e41a1c",
...
# middle = one track per repeat class
# top    = protein-coding genes
# ============================================================

# Only include repeat classes actually present in this PAR
repeat_classes_present <- names(repeat_colors)[
    names(repeat_colors) %in% as.character(unique(rep$broad_class))
]

# Preserve the same biologically meaningful/order defined by repeat_colors
repeat_classes_present <- repeat_classes_present[
    repeat_classes_present %in% names(repeat_colors)
]

# ------------------------------------------------------------
# Fixed bottom track: percent identity
# ------------------------------------------------------------

PID_CENTER <- 1
PID_YMIN   <- PID_CENTER - 0.30
PID_YMAX   <- PID_CENTER + 0.30


# ------------------------------------------------------------
# Repeat tracks
#
# Each repeat class gets its own horizontal row.
# ------------------------------------------------------------

repeat_track_table <- data.table(
    broad_class = repeat_classes_present,
    track_center = seq(
        from = 2,
        by = 1,
        length.out = length(repeat_classes_present)
    )
)

repeat_track_table[, ymin := track_center - 0.30]
repeat_track_table[, ymax := track_center + 0.30]


# Add track coordinates to each repeat annotation
rep <- merge(
    rep,
    repeat_track_table,
    by = "broad_class",
...
# ------------------------------------------------------------
# Fixed top track: genes
# ------------------------------------------------------------

GENE_CENTER <- max(repeat_track_table$track_center) + 1

GENE_YMIN <- GENE_CENTER - 0.30
GENE_YMAX <- GENE_CENTER + 0.30


# ------------------------------------------------------------
# Plot-wide y limits and labels
# ------------------------------------------------------------

track_breaks <- c(
    PID_CENTER,
    repeat_track_table$track_center,
    GENE_CENTER
)

track_labels <- c(
    "Percent identity",
    repeat_track_table$broad_class,
    "Protein-coding genes"
)

Y_MIN <- PID_CENTER - 0.60
Y_MAX <- GENE_CENTER + 0.60
...
p <- ggplot() +
    ...
    # Repeats
    #
    # Each broad repeat class is plotted on its own track.
    # --------------------------------------------------------
