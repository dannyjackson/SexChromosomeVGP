...
# NOTE: THERE IS NO CHROMOSOME 36 IN PRISTIS PECTINATA
1. Download WGS data
2. Align to Chelidae genome
3. Compare patterns of heterozygosity across the genome
## Could also use this genome from the same order: https://www.ncbi.nlm.nih.gov/datasets/genome/GCA_976551045.1/
...
#SBATCH --array=1-35
#SBATCH --output=/data/Wilson_Lab/projects/VGP_Phase_1_Sex_Chr_Project/jacksondan/datafiles/Sex_Chr_ID/Pristis/logs/%x_%A_%a.out
#SBATCH --error=/data/Wilson_Lab/projects/VGP_Phase_1_Sex_Chr_Project/jacksondan/datafiles/Sex_Chr_ID/Pristis/logs/%x_%A_%a.err

module load sratoolkit/3.3.0

cd /data/Wilson_Lab/projects/VGP_Phase_1_Sex_Chr_Project/jacksondan/datafiles/Sex_Chr_ID/Pristis || exit 1
...
#SBATCH --time=2:00:00
#SBATCH --gres=lscratch:500
#SBATCH --output=/data/Wilson_Lab/projects/VGP_Phase_1_Sex_Chr_Project/jacksondan/datafiles/Pristis/logs/index.out
#SBATCH --error=/data/Wilson_Lab/projects/VGP_Phase_1_Sex_Chr_Project/jacksondan/datafiles/Pristis/logs/index.err

module load bwa
REF_GENOME="/data/Wilson_Lab/data/VGP_genomes_phase1/genomes/Pristis_pectinata/ncbi_dataset/data/GCF_009764475.1/GCF_009764475.1_sPriPec2.1.pri_genomic.fna"
...
while read -r run; do
    java -jar $TRIMMOJAR PE \
        ${run}_out/${run}_1.fastq ${run}_out/${run}_2.fastq  \
        -baseout trimmed_fastas/${run}.trimmed.fq.gz \
...
#SBATCH --time=48:00:00
#SBATCH --gres=lscratch:500
#SBATCH --output=/data/Wilson_Lab/projects/VGP_Phase_1_Sex_Chr_Project/jacksondan/datafiles/Sex_Chr_ID/Pristis/logs/callvariants.out
#SBATCH --error=/data/Wilson_Lab/projects/VGP_Phase_1_Sex_Chr_Project/jacksondan/datafiles/Sex_Chr_ID/Pristis/logs/callvariants.err

module load bcftools

cd /data/Wilson_Lab/projects/VGP_Phase_1_Sex_Chr_Project/jacksondan/datafiles/Sex_Chr_ID/Pristis || exit 1

bamdir="/data/Wilson_Lab/projects/VGP_Phase_1_Sex_Chr_Project/jacksondan/datafiles/Sex_Chr_ID/Pristis/sorted_bam_files/"

REF_GENOME="/data/Wilson_Lab/data/VGP_genomes_phase1/genomes/Pristis_pectinata/ncbi_dataset/data/GCF_009764475.1/GCF_009764475.1_sPriPec2.1.pri_genomic.fna"

bcftools mpileup -Ou -f "$REF_GENOME" -a FORMAT/AD,DP,INFO/AD,SP "$bamdir"*.bam | bcftools call -mv -V indels > Pristis_snps_multiallelic.vcf
```
# Rename samples and zip vcf
```
sed -E '/^#CHROM/ s#(/[^[:space:]]*/)(SRR[0-9]+)\.sorted\.bam#\2#g' Pristis_snps_multiallelic.vcf > Pristis_snps_multiallelic.renamed.vcf
bgzip Pristis_snps_multiallelic.renamed.vcf
```
# Filter to sites with 25% coverage
```
bcftools view -i 'F_MISSING<0.75' Pristis_snps_multiallelic.renamed.vcf.gz -Oz -o Pristis_snps_multiallelic.filtered.vcf.gz
tabix -p vcf Pristis_snps_multiallelic.filtered.vcf.gz
```
# Remove scaffolds
```
zcat Pristis_snps_multiallelic.renamed.vcf.gz | grep -v 'NW_' > Pristis_snps_multiallelic.renamed.contigs.vcf
bgzip Pristis_snps_multiallelic.renamed.contigs.vcf
```
# Compute heterozygosity
zcat Pristis_snps_multiallelic.renamed.contigs.vcf.gz | grep -v "#" | awk '{print $1}' | sort -u > CONTIGS.txt
mkdir -p heterozygosity
module load vcftools

while read -r CHROM; do
    VCF=Pristis_snps_multiallelic.renamed.contigs.vcf.gz
...
gg <- ggplot(het, aes(x = chrom_plot, y = H_O, group = IND, color = H_X)) +
...
ggsave("heterozygosity_by_chromosome.Xgradient.stdH.pdf", gg, width = 12, height = 3, dpi = 300)
