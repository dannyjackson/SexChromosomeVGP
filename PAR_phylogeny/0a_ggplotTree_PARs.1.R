...
## (still used only to find PAR segments, but NOT to prune tips)
## --------------------------
...
## --------------------------
## Subset annotations: lineage ONLY
## (do NOT require a BED file here; we want blanks for missing PAR)
## --------------------------
ann_sub <- ann[get(lineage_col) == target_lineage]

if (nrow(ann_sub) == 0) {
  stop("No individuals in the target lineage found in annotation file.")
}
...
if (length(tips_to_keep) == 0) {
  stop("No matching accessions in the tree for this lineage.")
}
...
par_list <- lapply(seq_len(nrow(tip_meta)), function(i) {
  ...
  if (length(segs) == 0) {
    return(NULL)   # keep tip, just no PAR rows; we handle blanks later
  }
...
## ------------------------------------
## 2–3. PAR totals & bar plot with blanks + gray X
## ------------------------------------

## Tip order from the tree
tip_order <- p_tree$data %>%
  dplyr::filter(isTip) %>%
  arrange(y) %>%
  pull(label)

## Summarise PAR totals only for tips with data
par_totals_raw <- if (nrow(par_df) > 0) {
  par_df %>%
    group_by(EnglishName, Orders_English) %>%
    summarise(total_Mb = sum(len_Mb), .groups = "drop")
} else {
  data.frame(EnglishName = character(0),
             Orders_English = character(0),
             total_Mb = numeric(0))
}

## Join to ALL tips so species without PAR get total_Mb = 0
par_totals <- tip_meta %>%
  select(EnglishName, Orders_English) %>%
  left_join(par_totals_raw,
            by = c("EnglishName", "Orders_English")) %>%
  mutate(
    total_Mb = ifelse(is.na(total_Mb), 0, total_Mb),
    has_par  = total_Mb > 0,
    EnglishName = factor(EnglishName, levels = tip_order)
  )
...
p_bar <- ggplot(par_totals,
                aes(y = EnglishName,
                    x = total_Mb,
                    fill = Orders_English)) +
  ## Draw bars only for species with PAR data
  geom_col(data = subset(par_totals, has_par),
           width = 0.7) +
  ## Draw gray "X" for species with no PAR data, just right of x = 0
  geom_text(
    data = subset(par_totals, !has_par),
